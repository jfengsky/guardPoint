    ...
    return (
      <div style={{ padding: 10 }}>
        ...
        <Row style={{ paddingTop: 30 }}>
          <Col span={10} >
            {
              todoItem && <Collapse defaultActiveKey={['1', '2', '3', '4']}>
                <Panel header="任务" key="1">
                  <p><strong>{todoItem.title}</strong></p>
                </Panel>
                ...
                <Panel header="标签" key="4">
                  {
                    todoItem.tag.map((item: number) => {
                      return this.props.todoTags.map(({ value, color, label }: ITTodoTagOption) => {
                        if (value === item) {
                          return <Tag key={value} color={color}>{label}</Tag>
                        }
                      })
                    })
                  }
